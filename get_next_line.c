...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/28 00:56:21 by busyigit          #+#    #+#             */
/*   Updated: 2026/03/16 20:55:56 by busyigit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	left_str = gn_read_to_left_str(fd, left_str);
	if (!left_str)
		return (NULL);
	line = gn_get_line(left_str);
	if (!line)
	{
		free(left_str);
		left_str = NULL;
	...
	left_str = gn_new_left_str(left_str);
	return (line);
}
